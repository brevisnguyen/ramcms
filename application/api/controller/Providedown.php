...
class Providedown extends Base
{
    var $_param;
    ...
    public function vod()
    {
        ...
        if(empty($html) || $cache_time==0) {
            ...
            if (!empty($GLOBALS['config']['api']['vod']['from'])) {
                $where['vod_down_from'] = ['like', '%' . $GLOBALS['config']['api']['vod']['from'] . '%'];
            }
            ...
            $field = 'vod_id,vod_name,type_id,"" as type_name,vod_en,vod_time,vod_remarks,vod_down_from,vod_time';
        ...
        foreach($res['list'] as $k=>&$v){
            ...
            if($this->_param['ac']=='videolist' || $this->_param['ac']=='detail'){
                if ($GLOBALS['config']['api']['vod']['from'] != '') {
                    $arr_from = explode('$$$',$v['vod_down_from']);
                    $arr_url = explode('$$$',$v['vod_down_url']);
                    $arr_server = explode('$$$',$v['vod_down_server']);
                    $arr_note = explode('$$$',$v['vod_down_note']);

                    $key = array_search($GLOBALS['config']['api']['vod']['from'],$arr_from);
                    $res['list'][$k]['vod_down_from'] = $GLOBALS['config']['api']['vod']['from'];
                    $res['list'][$k]['vod_down_url'] = $arr_url[$key];
                    $res['list'][$k]['vod_down_server'] = $arr_server[$key];
                    $res['list'][$k]['vod_down_note'] = $arr_note[$key];
                ...
                if ($GLOBALS['config']['api']['vod']['from'] != '') {
                    ...
                    $res['list'][$k]['vod_down_from'] = str_replace('$$$', ',', $v['vod_down_from']);
                }
            }
        }
    ...
    public function vod_xml($res)
    {
        ...
        foreach($res['list'] as $k=>&$v){
            ...
            if($this->_param['ac']=='videolist' || $this->_param['ac']=='detail'){
                $tempurl = $this->vod_url_deal($v["vod_down_url"],$v["vod_down_from"],$GLOBALS['config']['api']['vod']['from'],'xml');
                ...
                if ($GLOBALS['config']['api']['vod']['from'] != ''){
                    ...
                    $xml .= '<dt>' . str_replace('$$$', ',', $v['vod_down_from']) . '</dt>';
                }
                $xml .= '<note><![CDATA[' . $v['vod_remarks'] . ']]></note>';
            }
            $xml .= '</video>';
