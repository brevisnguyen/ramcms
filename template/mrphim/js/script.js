function show_history(cookie_name) {
    ...
    if (history_data.length > 0) {
        ...
        history_html = '<li class="drop-tips">Không có lịch sử xem</li>';
    }
...
$(function () {
    ...
    if ($('.share-btn').length > 0) {
        ...
        clipboard.on('success', function (e) {
            show_tip('Thông tin chia sẻ được sao chép thành công, chia sẻ cho bạn bè cùng xem nhé ~');
        });
    ...
    if ($('.copy').length > 0) {
        ...
        down_clipboard.on('success', function (e) {
            show_tip('Địa chỉ tải xuống đã được sao chép thành công');
        });
    ...
    //请保留作者版权
    console.log('Mr.Phim phiên bản mã nguồn mở v1.0.1, mã nguồn này được cập nhật miễn phí vĩnh viễn, tác giả QQ: 602524950, nhóm trao đổi mẫu: 872882160, mã nguồn chỉ có thể dùng để học hỏi và trao đổi, cấm các hoạt động vi phạm pháp luật');
